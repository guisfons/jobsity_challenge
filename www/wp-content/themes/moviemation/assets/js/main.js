        ...
        if($('.actor').length) {
            ...
            <article class="movie-list__movie">
                <a href="/movies/movie/?${data.title.replace(/\s+/g, '-').replace(':', '').replace('"', '').replace('.', '').toLowerCase() + '?id=' + data.id}">
                    <figure>
                        <img src="${data.poster_path ? imgUrl + data.poster_path : 'http://via.placeholder.com/500x750'}" alt="${data.title}">
                    </figure>
                    <div>
                        <h3>${data.title}</h3>
                        <p>Release: <time datetime="${data.release_date}">${data.release_date}</time></p>
...
function getMovie(url) {
    $.ajax({
        ...
        <div class="wrapper movie-details__info">
            <figure class="movie-details__photo"><img src="${poster_path ? imgUrl + poster_path : 'http://via.placeholder.com/500x750'}" alt="${title}"></figure>
            ...
            if(i > 0 && i < 5) {
                ...
                <article>
                    <a href="/movies/movie/?${title.replace(/\s+/g, '-').replace(':', '').replace('"', '').replace('.', '').toLowerCase() + '?id=' + id}">
                        <figure>
                            <img src="${poster_path ? imgUrl + poster_path : 'http://via.placeholder.com/500x750'}" alt="${title}">
                        </figure>
                        <div>
                            <h3>${title}</h3>
                            <p>Release: <time datetime="${release_date}">${release_date}</time></p>
                            <p class="upcoming-movies__genre">Genre: ${genre[genre_ids]}</p>
                        </div>
                    </a>
                </article>
                `)
...
function getCast(url) {
    $.ajax({
        ...
        if($('.actor').length) {
            $.each(data.cast, function(i) {
                const {title, character, release_date, poster_path} = data.cast[i]
...
function getActor(url) {
    $.ajax({
        ...
        <div class="wrapper actor-details__info">
            <figure class="actor-details__photo"><img src="${profile_path ? imgUrl + profile_path : 'http://via.placeholder.com/500x750'}" alt="${name}"></figure>
            <div class="actor-details__header">
                <h1 class="actor-details__title">${name}</h1>
                <span class="actor-details__birthday">
                    ${birthdayFormated.toLocaleDateString('en-US')} ${deathday ? ' - ' + deathdayFormated.toLocaleDateString('en-US') : ''}
                </span>
                <article>
        ...
        <div class="actor-details__movies"><h3>Movies with Actor</h3></div>`)
    })
        ...
        if($('.actor').length) {
            $.each(data.profiles, function(i) {
                if(i < 9) {
                    ...
                    $('.actor-details__gallery').append('<figure><img src="'+imgUrl+'/'+file_path+'"></figure>')
                }
            })
        }
    })
...
function listActors(data) {
    $.each(data, function (i) {
            ...
            if (i < 10) {
                ...
                <article class="popular-actors__actor">
                    <a href="/actors/actor/?${name.replace(/\s+/g, '-').replace(':', '').toLowerCase() + '?id=' + id}">
                        <figure>
                            <img src="${profile_path ? imgUrl + profile_path : 'http://via.placeholder.com/500x750'}" alt="${name}">
                        </figure>
                        <h3>${name}</h3>
                    </a>
                </article>`)
        ...
        if ($('.actor-list').length) {
            ...
            <article class="actor-list__actor">
                <a href="/actors/actor/?${name.replace(/\s+/g, '-').replace(':', '').toLowerCase() + '?id=' + id}">
                    <figure>
                        <img src="${profile_path ? imgUrl + profile_path : 'http://via.placeholder.com/500x750'}" alt="${name}">
                    </figure>
